...
Bluster drop in replacement is not tested with the SMD2000 yet (the real Buster is).
...
Notes and Eratta
*-----------------------------------------------------------------------------*
Serial Port and Mouse Port are the same shape!  Now I know why Commodore used a 25 pin Serial port!  Be extra careful to not connect mouses to the serial port and serial devices to the mouse port otherwise there will be release of magic smoke!  Recommend using DB9 protector for serial port e.g.: https://www.thingiverse.com/thing:2121877

ROM:
A2000 rom socket is a 40 pin socket and can take a 512 kB rom 
(27C400 equivalent).

The SMD2000 ROM socket is a 42 pin socket wired for a 1MB or 2 MB rom (e.g. 27C800).  Pins are offset by 1, since extra address lines wired to CPU Bus A19 & A20 (this is normal for EPROMs).  

If using a Commodore rom or 27C400 eprom put it in the bottom of the socket, i.e. leave pins 1 and 42 on the socket empty (like on A500).  J519 & J520 do nothing.

If using a 1 MB or 2 MB rom with multiple images you can switch them using switches attached to CN519 & CN520; set J519 & J520 to 2-3.  Many users want a Kickstart switch with KS1.3 and KS3.1/3.2; this can be achieved using a 27C800 with KS1.3 in the lower half and KS3.1 or 3.2 in the top half and connecting a switch to CN519.

If using a 1MB or 2MB custom rom set J519 & J520 to 1-2.

LEDs:
LEDs have been added to the computer.  There are currently three ways to see the power LED, and two for the floppy drive LED (plus the LEDs on the floppy drives themselves).
Power LEDs:
Case: this is the same as in the A2000.  Populate Q302, R307, & C919.  If R308 is not populated the case LED will flash on and off instead of bright and dim when the Guru is about to meditate.  You can populate R308 if you want the LED to "dim" instead of switch off.  Commodore used a 200 ohm resistor in the A2000 and a 100 ohm resisitor on the A500 but optimal size will vary for each LED.
...
FDD LEDs:
Floppy LEDs added same way as A500 (inverted _MTR0D).  Most users will not want to add a Floppy LED to the case so these components are marked "Do Not Populate".
...
NB the LEDs have changed since the prototype.  The extra LED options (other than for the case LED) are UNTESTED but there is no reason they will not work.

Motherboard mounting holes had to be moved a little after the first prototype, and as a result the board was made 4 mm wider than "DTX" specification but it still fits in the ML09 case.  Be careful if you intend to fit it in a tighter case.
...
The motherboard is expensive to make, being four layers.  I used JLCPCB, the cost was about $170 AUD for the minimum order of five boards, or a bit under $35 per board.  I chose green because it was the cheapest, they will do other colours for extra cost.  This includes an extra fee of $40 AUD due to the requirement for a 4-Wire Kelvin Test, due to the small size of the vias (this was mandatory from JLC).   Given the board is close to the limitations of JLCPCB I'm OK with that.  Even with the Kelvin testing it's less than half the price of PCBWaaaaaaay at USD 187.
